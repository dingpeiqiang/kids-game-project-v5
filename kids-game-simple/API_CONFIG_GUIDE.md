...
部署到云服务器后，kids-game-simple 仍然访问 `localhost:8080`，导致 API 请求失败。
...
[nginx] 复制 kids-game-simple.conf → /etc/nginx/conf.d/default.conf
...
```bash
scp docker-images/kids-game-simple.tar root@8.136.156.190:/tmp/
```
...
# 选择选项 5) kids-game-simple 镜像加载 + 部署
```
...
```bash
# 在服务器上
docker exec kids-game-kids-game-simple cat /etc/nginx/conf.d/default.conf | grep proxy_pass
...
2. 查看请求 URL 是否为 `/api/...`
3. 检查响应状态码（200 = 成功）
4. 查看 Nginx 日志：`docker logs kids-game-kids-game-simple`
