...
const oldStartBlock = `    // 开始/结束界面
    if (!state.gameStarted) {
      ...
      ctx.fillText('\\u{1F3F0} RPG塔防射击', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 60)
...
const newStartBlock = `    // 开始界面（与 dragonShooter 统一模板风格）
    if (!state.gameStarted) {
      ...
      ctx.font = 'bold 24px sans-serif'
      ctx.textAlign = 'center'
      ctx.fillText('\\u{1F3F0} RPG塔防射击', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 20)
      ...
      ctx.fillText('\\u{1F916} 自动射击！只需移动角色！', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 30)
...
content = content.replace(oldStartBlock, newStartBlock);

// 完整替换结束界面
const oldEndBlock = `    if (state.gameEnded) {
      ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'
      ...
      ctx.textAlign = 'center'
      ctx.fillText('\\u{1F3C6} 游戏结束', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 40)
...
const newEndBlock = `    // 游戏结束界面（与 dragonShooter 统一模板风格）
    if (state.gameEnded) {
...
console.log('✅ 文件修改完成！');
console.log('开始界面和结束界面已统一为 dragonShooter 模板风格');
