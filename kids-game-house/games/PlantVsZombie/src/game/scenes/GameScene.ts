...
export class GameScene extends BaseScene {
  ...
  private debugText: Phaser.GameObjects.Text | null = null; // 调试文本
  ...
  protected onUpdate(time: number, delta: number): void {
    ...
    if (this.debugText && GAME_CONFIG.DEBUG) {
      ...
      // 计算预期的网格位置
      let expectedGrid = 'N/A';
      if (gridPos) {
        const { OFFSET_X, OFFSET_Y, CELL_WIDTH, CELL_HEIGHT } = GRID_CONFIG;
        const expectedX = OFFSET_X + gridPos.col * CELL_WIDTH;
        const expectedY = OFFSET_Y + gridPos.row * CELL_HEIGHT;
        expectedGrid = `(${gridPos.row},${gridPos.col})@[${expectedX}-${expectedX+CELL_WIDTH},${expectedY}-${expectedY+CELL_HEIGHT}]`;
      }
      
      this.debugText.setText([
        `Screen: (${pointer.x.toFixed(0)}, ${pointer.y.toFixed(0)})`,
        `World: (${pointer.worldX.toFixed(0)}, ${pointer.worldY.toFixed(0)})`,
        `Camera Scroll: (${camera.scrollX.toFixed(0)}, ${camera.scrollY.toFixed(0)})`,
        `Grid: ${expectedGrid}`,
        `Preview Pos: ${this.plantPreview ? `(${this.plantPreview.x.toFixed(0)}, ${this.plantPreview.y.toFixed(0)})` : 'N/A'}`
      ].join('\n'));
    }
  ...
  private createLawnMower(x: number, y: number, row: number): void {
    ...
    // 关键修复：将割草机添加到 gameContainer，使其跟随摄像机滚动
    if (this.gameContainer) {
  ...
  private updatePlantPreview(): void {
    ...
    // 关键修复：考虑摄像机滚动偏移
    // pointer.worldX 已经包含了摄像机偏移，但我们需要的是相对于游戏世界的绝对坐标
    // 由于网格是在世界坐标系中创建的（没有跟随摄像机），我们需要使用 worldX/worldY
    const worldX = pointer.worldX;
    const worldY = pointer.worldY;
    
    const gridPos = this.gridSystem.screenToGrid(worldX, worldY);

    if (gridPos) {
      // Snap to grid - 计算网格中心点
      const { OFFSET_X, OFFSET_Y, CELL_WIDTH, CELL_HEIGHT } = GRID_CONFIG;
      ...
      this.plantPreview.setPosition(worldX, worldY);
      const rect = this.plantPreview.list[0] as Phaser.GameObjects.Rectangle;
