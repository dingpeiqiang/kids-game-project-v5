export default class KeyMap {
  constructor(character) {
    ...
    this.scene = character.scene;  // 保存 scene 引用
    ...
    let keys = {
      ...
      inventory: {
        ability: this.toggleInventory.bind(this),  // 绑定 this
        icon: 'inventory'
      },
      questLog: {
        ability: this.toggleQuestLog.bind(this),  // 绑定 this
        icon: 'quest'
      },
      equipment: {
        ability: this.toggleEquipment.bind(this),  // 绑定 this
        icon: 'equipment'
      }
    }
    ...
    this.execute = function(num) {
      if (!keys[num] || !keys[num].ability) return;
