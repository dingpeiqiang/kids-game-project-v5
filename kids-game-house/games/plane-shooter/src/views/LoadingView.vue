...
onMounted(async () => {
  console.log('🎯 LoadingView mounted, current route:', router.currentRoute.value.path)
  try {
    for (const step of loadingSteps) {
      ...
      console.log(`⏳ ${step.percent}% - ${step.text}`)
      await new Promise(resolve => setTimeout(resolve, 80 + Math.random() * 40))
    }
    
    console.log('✅ 加载完成，准备跳转到 /start')
    setTimeout(() => {
      console.log('🚀 执行路由跳转...')
      router.push('/start').then(() => {
        console.log('✅ 路由跳转成功:', router.currentRoute.value.path)
      }).catch(err => {
