...
// Declare ROT globally to avoid import issues with Snowpack
declare const ROT: any
...
export default class WorldGenerator extends Phaser.GameObjects.Container {
  ...
  public staticGenerate() {
    ...
    // Use global ROT object from window
    const ROTGlobal = (window as any).ROT
    if (!ROTGlobal) {
      console.error('ROT.js not loaded!')
      return
    }
    
    const digger = new ROTGlobal.Map.Cellular(
