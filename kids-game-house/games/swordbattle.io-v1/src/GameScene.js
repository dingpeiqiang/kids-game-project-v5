...
async function loadConfig() {
  try {
    ...
    configData = { CAPTCHASITE: '', localServer: true };
  }
}
...
class GameScene extends Phaser.Scene {
	...
	create() {
		...
		//recaptcha
		grecaptcha.ready(() =>{
			grecaptcha.execute(configData.CAPTCHASITE, {action: "join"}).then((thetoken) => {
		...
		// let thetoken = "subtocodergautam";
		  ...
		  if(this.spectating) return;
			...
			this.UICam.ignore([this.mePlayer, this.meBar.bar, this.meSword, this.background, this.meChat]);
				...
				bushesData.locations.forEach((l,i) => {
          if(i%2==0) return;
...
// }
	...
	this.home.btn.y = this.statsText.y + this.statsText.displayHeight - Math.min(200*this.playAgain.btn.scaleY, 80);
            ...
            if(this.spectating) return;
				...
				this.socket = io(configData.localServer?document.location.host.includes("localhost")  ? "ws://localhost:3000" : (isPrivateIP(document.location.host) ? "ws://"+document.location.host : "wss://"+document.location.host):server);
...
this.lastCountriesClear = Date.now();
	...
	setupOfflineMode() {
		...
		// 添加剑
		this.playerSword = this.add.image(4000, 4000, 'playerSword');
		this.playerSword.setScale(1.5);
