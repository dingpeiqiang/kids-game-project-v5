...
// ── localStorage keys ──────────────────────────────────────────────────────
const THEME_STORAGE_KEY = 'forest-guardian-theme-id'
...
export const useThemeStore = defineStore('theme', () => {
  ...
  async function loadDefaultTheme(): Promise<void> {
    // ⚠️ 临时方案：由于去掉了认证，这里直接加载本地默认 GTRS
    try {
      const response = await fetch('/themes/top_down/GTRS.json')
      if (!response.ok) throw new Error('Failed to load local GTRS')
      
      const gtrs = await response.json()
      gtrsRawJson.value    = JSON.stringify(gtrs)
      gtrsData.value       = gtrs
      currentThemeId.value = 'top_down'
      error.value          = null

      console.log('✅ 本地 GTRS 主题加载成功:', gtrs.themeInfo?.themeName)
    } catch (err: any) {
      console.error('❌ 本地主题加载失败:', err.message || err)
      throw err
    }
  }
