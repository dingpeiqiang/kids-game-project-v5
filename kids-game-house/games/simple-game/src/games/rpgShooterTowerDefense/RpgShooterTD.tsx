...
import { updateTurrets, drawTurret, selectBuildType, canPlaceTurret, placeTurret } from './turrets'
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (clicked && !s.buildMode.active) {
        const config = TURRET_CONFIGS[clicked.type]
        const refund = Math.floor(config.cost * 0.5)
        s.resources.crystals += refund
        s.turrets = s.turrets.filter(t => t.id !== clicked.id)
        s.floatTexts.push({ text: `💰 +${refund}`, x: clicked.x, y: clicked.y - 20, life: 1.5, color: '#FFD700', size: 16, vy: -0.5 })
        return
      }
        ...
        {uiState.buildMode && (
          <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 pointer-events-auto">
            <div className="bg-black bg-opacity-85 rounded-xl p-3 shadow-xl">
              <div className="text-center text-white text-xs mb-2 opacity-70">点击地图放置炮台</div>
              <div className="grid grid-cols-4 gap-2">
                <button
                  onClick={() => handleSelectTurret('laser')}
                  className={`px-2 py-3 rounded-lg flex flex-col items-center transition-all ${
                    uiState.selectedTurret === 'laser'
                      ? 'bg-cyan-500 ring-2 ring-white'
                      : 'bg-gray-700 hover:bg-gray-600'
                  } text-white text-xs`}
                >
                  <span className="text-xl">🔫</span>
                  <span className="mt-1">激光</span>
                  <span className="opacity-70">40💎</span>
                </button>
                <button
                  onClick={() => handleSelectTurret('missile')}
                  className={`px-2 py-3 rounded-lg flex flex-col items-center transition-all ${
                    uiState.selectedTurret === 'missile'
                      ? 'bg-red-500 ring-2 ring-white'
                      : 'bg-gray-700 hover:bg-gray-600'
                  } text-white text-xs`}
                >
                  <span className="text-xl">🚀</span>
                  <span className="mt-1">导弹</span>
                  <span className="opacity-70">80💎</span>
                </button>
                <button
                  onClick={() => handleSelectTurret('frost')}
                  className={`px-2 py-3 rounded-lg flex flex-col items-center transition-all ${
                    uiState.selectedTurret === 'frost'
                      ? 'bg-blue-400 ring-2 ring-white'
                      : 'bg-gray-700 hover:bg-gray-600'
                  } text-white text-xs`}
                >
                  <span className="text-xl">❄️</span>
                  <span className="mt-1">冰冻</span>
                  <span className="opacity-70">50💎</span>
                </button>
                <button
                  onClick={() => handleSelectTurret('lightning')}
                  className={`px-2 py-3 rounded-lg flex flex-col items-center transition-all ${
                    uiState.selectedTurret === 'lightning'
                      ? 'bg-yellow-500 ring-2 ring-white'
                      : 'bg-gray-700 hover:bg-gray-600'
                  } text-white text-xs`}
                >
                  <span className="text-xl">⚡</span>
                  <span className="mt-1">闪电</span>
                  <span className="opacity-70">120💎</span>
                </button>
              </div>
              {/* 数字键提示 */}
              <div className="text-center text-white text-xs mt-2 opacity-50">数字键 1-4 快速选择</div>
            </div>
          </div>
        )}
