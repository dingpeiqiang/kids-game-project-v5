...
function applySelectedPowerUp(state: GameState, card: PowerUpCard) {
  ...
  switch (card.type) {
    ...
    case 'rapidFire':     // 迅击弹：攻速大幅提升（叠加 shootCooldown 减少）
      ...
      addOrRefreshBuff(state, 'rapidFire', '迅击弹', '⚡', '#FFD700', 10)
      pushText('⚡ 迅击弹！')
      break
    ...
    case 'heavyHit':    // 重击弹：单发伤害大幅提高
      ...
      addOrRefreshBuff(state, 'heavyHit', '重击弹', '💪', '#FF4500', 10)
      pushText('💪 重击弹！')
      break
    case 'rapidBurst':    // 连射增幅：短时间极速连射（临时极短冷却）
      ...
      addOrRefreshBuff(state, 'rapidBurst', '连射增幅', '🔥', '#FF4500', 10)
      pushText('🔥 连射增幅！')
      break
    case 'autoAim':       // 精准锁定：子弹自动吸附龙身
      S.autoAimStacks = (S.autoAimStacks || 0) + 1
      state.bulletPierce += 1
      addOrRefreshBuff(state, 'autoAim', '精准锁定', '🎯', '#00CED1', 10)
      pushText('🎯 精准锁定！')
      break

    // ── 二，范围爆发类（6）─────────────────────────────────────
    case 'blast':         // 爆裂冲击：小范围爆炸（持续10秒，每秒触发）
      S.blastStacks = (S.blastStacks || 0) + 1
      addOrRefreshBuff(state, 'blast', '爆裂冲击', '💥', '#FF6347', 10)
      audioService.explosion()
      pushText('💥 爆裂冲击！')
      break
    case 'slash':         // 横向横扫：横向范围伤害（持续10秒）
      S.slashStacks = (S.slashStacks || 0) + 1
      addOrRefreshBuff(state, 'slash', '横向横扫', '⚔️', '#DDA0DD', 10)
      pushText('⚔️ 横向横扫！')
      break
    case 'ringWave':      // 环形震荡：定时释放环形冲击波（叠加 timer）
      S.ringWaveStacks = (S.ringWaveStacks || 0) + 1
      S._ringWaveTimer = (S._ringWaveTimer || 0) + 3
      addOrRefreshBuff(state, 'ringWave', '环形震荡', '🌊', '#00CED1', 10)
      pushText('🌊 环形震荡！')
      break
    case 'windPressure':  // 全屏风压：大范围减速龙
      S.windPressureStacks = (S.windPressureStacks || 0) + 1
      state.slowAllTimer = Math.max(state.slowAllTimer, (state.slowAllTimer || 0) + 10)
      addOrRefreshBuff(state, 'windPressure', '全屏风压', '🌪️', '#87CEEB', 10)
      pushText('🌪️ 全屏风压！')
      break
    case 'chainBlast':    // 分段爆破：命中后连锁爆破（持续10秒）
      S.chainBlastStacks = (S.chainBlastStacks || 0) + 1
      addOrRefreshBuff(state, 'chainBlast', '分段爆破', '⛓️', '#808080', 10)
      pushText('⛓️ 分段爆破！')
      break
    case 'splash':        // 范围溅射：伤害小幅溅射周围（持续10秒）
      S.splashStacks = (S.splashStacks || 0) + 1
      addOrRefreshBuff(state, 'splash', '范围溅射', '💦', '#1E90FF', 10)
      pushText('💦 范围溅射！')
      break

    // ── 三，持续压制类（5）─────────────────────────────────────
    case 'burn':          // 持续灼烧：每帧对所有龙附加灼烧层数（持续10秒）
      S.burnStacks = (S.burnStacks || 0) + 1
      addOrRefreshBuff(state, 'burn', '持续灼烧', '🔥', '#FF4500', 10)
      pushText('🔥 持续灼烧！')
      break
    case 'slowField':     // 迟缓领域：降低整条龙移动速度
      S.slowFieldStacks = (S.slowFieldStacks || 0) + 1
      state.slowAllTimer = Math.max(state.slowAllTimer || 0, 10)
      addOrRefreshBuff(state, 'slowField', '迟缓领域', '❄️', '#87CEFA', 10)
      pushText('❄️ 迟缓领域！')
      break
    ...
    case 'energyField':   // 能量涌动：角色周围持续生成伤害圈（叠加）
      ...
      addOrRefreshBuff(state, 'energyField', '能量涌动', '⚡', '#FFD700', 10)
      pushText('⚡ 能量涌动！')
      break
    case 'defDown':       // 减防光环：降低全场巨龙防御（持续10秒）
      S.defDownStacks = (S.defDownStacks || 0) + 1
      state.bulletDamage = Math.floor(state.bulletDamage * 1.15)
      addOrRefreshBuff(state, 'defDown', '减防光环', '🛡️', '#B0C4DE', 10)
      pushText('🛡️ 减防光环！')
      break

    // ── 四，特殊斩杀类（3）─────────────────────────────────────
    case 'slashBlade':    // 断龙利刃：概率直接斩断一节龙身（持续10秒）
      S.slashBladeStacks = (S.slashBladeStacks || 0) + 1
      addOrRefreshBuff(state, 'slashBlade', '断龙利刃', '🗡️', '#8B0000', 10)
      // 立即触发一次斩断
      ...
      pushText('🗡️ 断龙利刃！')
      break
    case 'executeWave':   // 斩杀剑气：纵向长剑气贯穿路线（持续10秒）
      S.executeWaveStacks = (S.executeWaveStacks || 0) + 1
      addOrRefreshBuff(state, 'executeWave', '斩杀剑气', '⚡', '#9370DB', 10)
      audioService.lightning()
      pushText('⚡ 斩杀剑气！')
      break
    case 'crit':          // 极限暴击：大幅提升暴击概率（持续10秒）
      S.critStacks = (S.critStacks || 0) + 1
      state.bulletDamage = Math.floor(state.bulletDamage * 1.5)
      addOrRefreshBuff(state, 'crit', '极限暴击', '💢', '#FF1493', 10)
      pushText('💢 极限暴击！')
      break
  }
  audioService.buff()
