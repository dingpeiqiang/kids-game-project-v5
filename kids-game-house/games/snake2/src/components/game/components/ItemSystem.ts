...
export class ItemSystem {
  ...
  private trySpawnItem(): void {
    ...
    if (activeItems.length >= this.config.maxActiveItems) {
      if (this.config.debugMode) {
        console.log('🎁 [ItemSystem] 当前道具数量已达上限:', activeItems.length, '/', this.config.maxActiveItems)
      }
      return
    }
  ...
  private activateItemEffect(item: GameItem): void {
    ...
    switch (item.type) {
      ...
      case 'length_reduce':
        // 移除 3 节蛇身（一次性效果）
        this.emitEffectEvent('length_reduce', { segmentsToRemove: 3 })
        break
      
      case 'shield':
        // 无敌 10 秒
        this.emitEffectEvent('shield', { duration: 10000 })
        break
      ...
      case 'double_score':
        // 分数 x2，持续 10 秒
        this.emitEffectEvent('double_score', { multiplier: 2, duration: 10000 })
        break
    }
  ...
  private emitEffectEvent(effectType: string, params: any): void {
    ...
    eventBus.emit({
      ...
      timestamp: Date.now()
    } as any)  // 👉 暂时使用 any 绕过类型检查
  }
  
  /**
   * 🎁 处理实体与道具的碰撞事件（来自 EventBus）
   * 
   * @param event ENTITY_ITEM_COLLISION 事件对象
   */
