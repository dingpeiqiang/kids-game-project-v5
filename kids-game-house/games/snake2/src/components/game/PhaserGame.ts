...
export class SnakePhaserGame {
  ...
  initializeEntitySystem(cellSize?: number, gridCols?: number, gridRows?: number): void {
    ...
    // 创建 SnakePhaserGameV2 实例（使用已导入的类型）
   ...
   * 
   * @remarks
   * 将 SnakePhaserGameV2 的渲染输出转换为 Phaser Graphics
   * 然后生成纹理显示到场景中
   */
  renderEntitiesToPhaser(): void {
    ...
    if (!this.entitiesTexture) {
      ...
      this.entitiesTexture = this.scene.textures.createCanvas('entities_texture_v2', gameWidth, gameHeight)
...
// ⭐ 将 SnakePhaserGame 作为默认导出，并提供 PhaserGame 别名
