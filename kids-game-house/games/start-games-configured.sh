...
#
# 用法:
#   ./start-games-configured.sh              # 开发模式启动
#   ./start-games-configured.sh --production  # 生产模式（构建后启动，更省内存）
# ========================================
...
for arg in "$@"; do
    case "$arg" in
        --production|-p) PRODUCTION_MODE=true ;;
    esac
done
...
echo "$ENABLED_GAMES" | while IFS= read -r game; do
    ...
    if [ "$PRODUCTION_MODE" = true ]; then
        log_info "启动游戏服务（生产模式）..."
        # 生产模式：构建后使用 preview（更省内存，50%+）
        npm run build 2>&1 | tail -5
        nohup npm run preview -- --host 0.0.0.0 --port $GAME_PORT > "$SCRIPT_DIR/logs/${GAME_ID}.log" 2>&1 &
    else
        log_info "启动游戏服务（开发模式）..."
