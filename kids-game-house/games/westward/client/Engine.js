...
Engine.create = function(){
    ...
    try {
        ...
        // 使用默认的简单配置
        Engine.tilesetData.atlas = {};
        Engine.tilesetData.config = { grassPrefix: 'grass', grassSize: 3 };
        Engine.tilesetData.shorthands = {};
    }
...
Engine.configEngine = function(){
    ...
    // 添加默认的建造配方，避免空值错误
    if (!Engine.config.defaultBuildRecipes) {
        Engine.config.defaultBuildRecipes = [];
    }
};
...
Engine.initWorld = function(data){
    ...
    */
    ...
    // 纯前端模式：使用默认区域名称
    var regionName = 'Greenwood';
    if (data && data.region !== undefined && regionsData[data.region]) {
        regionName = regionsData[data.region].name;
    }
    Engine.setlCapsule.setText(regionName);
...
Engine.displayChunk = function(id){
    if (UI.offlineMode) {
        ...
        try {
            // 从 AOI 编号转换为坐标
            var aoiCoords = Utils.AOIcoordinates(id);
            if (!aoiCoords || aoiCoords.x === undefined || aoiCoords.y === undefined) {
                console.warn('无法获取 AOI 坐标', id);
                return;
            }
            var chunkX = aoiCoords.x;
            var chunkY = aoiCoords.y;
            
            // 生成或获取模拟地图块
            var chunkData = OfflineGameData.generateChunk(chunkX, chunkY);
            
            // 设置 chunkData 的 id 格式匹配预期
            chunkData.id = id;
            chunkData.x = chunkX;
            chunkData.y = chunkY;
            
            // 确保有正确的格式
            chunkData.default = chunkData.default || 'grass';
            chunkData.layers = chunkData.layers || [[]];
            chunkData.decor = chunkData.decor || [];
            chunkData.wood = chunkData.wood || [];
            
            console.log('纯前端模式：显示地图块', id, '(', chunkX, ',', chunkY, ')');
            Engine.drawChunk(chunkData, id);
        } catch (e) {
            console.error('显示地图块时出错:', e, 'id:', id);
        }
        return;
    }
...
Engine.computePath = function(position,nextTo){
    ...
    try {
        ...
        if(trim.trimmed && Engine.player.setDestinationAction) Engine.player.setDestinationAction(0);
        path = trim.path;
