...
export class VirtualJoystick {
  ...
  /** 布局编辑：从持久化配置恢复摇杆锚点与尺寸 */
  applyLayoutAnchor(joyCfg: {
    x?: number
    y?: number
    radius?: number
    knobRadius?: number
  }): void {
    if (joyCfg.x != null) this.config.x = joyCfg.x
    if (joyCfg.y != null) this.config.y = joyCfg.y
    if (joyCfg.radius != null) this.config.radius = joyCfg.radius
    if (joyCfg.knobRadius != null) this.config.knobRadius = joyCfg.knobRadius
  }
