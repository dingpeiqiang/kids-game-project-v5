/**
 * 统一应用路由（终端大厅 + 管理/家长门户，以 simple 壳为入口）
 */
import { createRouter, createWebHistory } from 'vue-router';
import type { RouteRecordRaw } from 'vue-router';
import { getCurrentUserType, isLoggedIn } from '@/utils/auth';
import { getDefaultAdminLanding } from '@kids-game/shared';
import { isEmbeddedCanvasGame } from '@/games/embeddedCanvasGames';
import { setAppRouter } from '@shell/router/navigation';
import { adminPortalRoutes, applyAdminPortalGuards, portalRole } from '@shell/router/adminRoutes';

const LobbyTab = () => import('@shell/views/lobby/LobbyTabView.vue');
const PlatformShell = () => import('@shell/layouts/PlatformShell.vue');
...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    path: '/game/:type/play',
    name: 'Game',
    component: () => import('@shell/views/GamePlayHost.vue'),
    ...
    : []),
  {
    ...
    redirect: () => {
      ...
      if (role) return getDefaultAdminLanding(role);
      return '/home';
    },
  },
];
...
router.beforeEach((to, _from, next) => {
  ...
  if (title) {
    const suffix = to.path.startsWith('/admin') ? '星光游学管理端' : '星光游学';
  ...
  if (applyAdminPortalGuards(to, next, loggedIn)) {
    return;
  }
