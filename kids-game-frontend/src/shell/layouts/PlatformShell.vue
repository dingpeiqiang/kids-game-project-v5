...
import type { PlatformContext } from '@shell/app/types'
import type { Game } from '@shell/types'
import { getLevelByExp } from '@shell/types/user'
import { AuthModal, MePanel, injectUserStyles } from '@shell/services/userUI'
import { showDailyPop, closeDailyPop } from '@shell/app/ui'
import {
  renderGameCards,
  ...
  renderFavoritesPage,
  refreshBestScores,
} from '@shell/app/gameCards'
import { launchGame } from '@shell/app/gameSession'
import {
  showRank,
  showRankForGame,
  closeRank,
...
import { bindEvents, bindGameCallbacks } from '@shell/app/events'
import { bindEconomyButtons, fetchTasksForBanner, bannerTaskView } from '@shell/app/economyUI'
import { lobbyPathForTab, navigateTo } from '@shell/router/navigation'
import { isLoggedIn as isSharedAuthLoggedIn } from '@/utils/auth'
import '@shell/styles/main.css'
import '@shell/styles/game-shell.css'
...
async function onBannerClaim(task: BannerTask) {
  ...
  const { apiTaskClaim } = await import('@shell/services/apiClient')
  ...
  if (claim.ok && claim.data && (claim.data as Record<string, unknown>).success === true) {
    ...
    const { showToast } = await import('@shell/services/userUI')
