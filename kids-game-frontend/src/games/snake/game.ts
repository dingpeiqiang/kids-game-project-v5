...
import type { ActivePowerup } from '@shell/services/powerupManager'
...
import { getCanvasPaletteForGame } from '@shell/utils/GTRSThemeApplier'
  ...
  function moveSnake() {
    ...
    for (let i = foods.length - 1; i >= 0; i--) {
      if (foods[i].x === head.x && foods[i].y === head.y) {
        ...
        gameActions.addScore(finalPts, head.x * CELL + OFFSET_X, head.y * CELL + OFFSET_Y)
...
const lifecycle = createCanvasGameLifecycle('snake', startSnakeLifecycle)

export const initSnake = lifecycle.init
export const destroySnake = lifecycle.destroy
