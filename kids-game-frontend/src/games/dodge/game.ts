...
import { bindHorizontalSwipePan } from '@shell/platform/mobileControls'
...
function startDodgeLifecycle(lifecycleCtx: GameLifecycleContext): GameLifecycle {
  ...
  function activatePowerup(p: any) {
    ...
    switch (p.type) {
      ...
      case 'score3x':
        double = 12  // 长时间双倍
        gameActions.addScore(50, p.x, p.y)
        score += 50
        break
      case 'bomb':
        // 炸弹效果：清除所有障碍并加分
...
const lifecycle = createCanvasGameLifecycle('dodge', startDodgeLifecycle)

export const initDodge = lifecycle.init
export const destroyDodge = lifecycle.destroy
