...
do_action() {
    case "$1" in
        1)  # 前端镜像加载 + 部署
            ...
            cleanup_dangling_images
            echo ">>> 启动容器..."
            docker compose -f "$COMPOSE_FILE" up -d --no-build frontend > /dev/null 2>&1
            # 等待容器进入 running 状态
            for i in $(seq 1 10); do
                RUNNING=$(docker inspect --format='{{.State.Running}}' kids-game-frontend 2>/dev/null)
                [ "$RUNNING" = "true" ] && break
                sleep 1
            done
            sleep 3
        ...
        3)  # 后端镜像加载 + 部署
            ...
            cleanup_dangling_images
            echo ">>> 启动容器..."
            docker compose -f "$COMPOSE_FILE" up -d --no-build backend > /dev/null 2>&1
            # 等待容器进入 running 状态
            for i in $(seq 1 20); do
                RUNNING=$(docker inspect --format='{{.State.Running}}' kids-game-backend 2>/dev/null)
                [ "$RUNNING" = "true" ] && break
                sleep 1
            done
            sleep 5
        ...
        5)  # simple-game 镜像加载 + 部署
            ...
            cleanup_dangling_images
            echo ">>> 启动容器..."
            docker compose -f "$COMPOSE_FILE" up -d --no-build simple-game > /dev/null 2>&1
            for i in $(seq 1 10); do
                RUNNING=$(docker inspect --format='{{.State.Running}}' kids-game-simple-game 2>/dev/null)
                [ "$RUNNING" = "true" ] && break
                sleep 1
            done
            sleep 3
        ...
        7)  # 全量部署
            ...
            cleanup_dangling_images
            echo ">>> 启动所有服务..."
            docker compose -f "$COMPOSE_FILE" up -d --no-build > /dev/null 2>&1
            # 等待所有容器进入 running 状态
            for i in $(seq 1 20); do
                BACKEND_RUNNING=$(docker inspect --format='{{.State.Running}}' kids-game-backend 2>/dev/null)
                FRONTEND_RUNNING=$(docker inspect --format='{{.State.Running}}' kids-game-frontend 2>/dev/null)
                [ "$BACKEND_RUNNING" = "true" ] && [ "$FRONTEND_RUNNING" = "true" ] && break
                sleep 1
            done
            sleep 5
            echo ""
