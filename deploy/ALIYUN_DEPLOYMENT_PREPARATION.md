...
- ✅ `deploy/docker/nginx/frontend.conf` - 前端 Nginx 配置
  - SPA 路由支持
  - Gzip 压缩
  - 静态资源缓存
  - API 反向代理
...
- ✅ `deploy/docker/mysql/init.sql` - 自动初始化脚本
  - 创建所有必要的表结构
  - 插入默认管理员账号
  - 插入示例游戏数据
  - 配置索引和优化
...
- ✅ `deploy/deploy.sh` - Linux/Mac 一键部署脚本
  - 依赖检查
  - 配置验证
  - 构建和启动
  - 日志查看
...
- ✅ `deploy/deploy.bat` - Windows 一键部署脚本
  - 相同功能的 Windows 版本
...
# 设置定时备份（每天凌晨 2 点）
crontab -e
# 添加：0 2 * * * cd /opt/kids-game-project-v5 && ./deploy/deploy.sh backup >> /var/log/db-backup.log 2>&1
```
