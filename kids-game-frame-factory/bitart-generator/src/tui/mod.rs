                    ...
                    match exporter::save_chain_pngs(&tile_refs, &base_dir) {
                        Ok(paths) => {
                            self.status_message = format!(
                                "Saved {} tiles to {}!",
                                paths.len(),
                                base_dir.replace('\\', "/") // Normalize for display
                            );
                        }
                    ...
                    ExportMode::Png => {
                        if let Some(ref canvas) = self.canvas {
                            match exporter::save_png(canvas, &path) {
                                Ok(()) => self.status_message = format!("Saved to {}!", path.replace('\\', "/")),
                                Err(e) => self.status_message = format!("Save failed: {}", e),
                            }
                        }
                    }
...
pub fn run() -> std::io::Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        if !supports_ansi() {
            eprintln!("Warning: Your terminal may not fully support ANSI colors. Consider using Windows Terminal for best experience.");
        }
    }
